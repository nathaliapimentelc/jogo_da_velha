...
def get_color(icon):
    return ft.colors.GREEN_ACCENT_400 if ft.icons.CIRCLE_OUTLINED == icon else ft.colors.BLUE_ACCENT_400
...
def main(page: ft.Page):
    ...
    def switch_player(item : str):
        ...
        players.controls.append(ft.Row(
            [
                ft.Icon(name=icon, size=50, tooltip=name, color=get_color(icon)),
                ft.Text(name)
            ]
        ))
        select_icon(icon)
        ...
        if len(players.controls) == 2:
            ...
            value = {
                'topic': 'create_board',
                'value': f"{player_1_name} {player_1_icon} {player_2_name} {player_2_icon}"
            }
